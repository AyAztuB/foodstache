...
class AddRecetteFragment : Fragment() {
    ...
    private fun UploadPost() {
        if (binding.addRecetteTitle.text.toString()==""){
            Toast.makeText(context, "Title is missing", Toast.LENGTH_LONG).show()
        }
        else{
            var vide=false
            for (text in 0 until binding.list1.childCount) {
                var oui=binding.list1.getChildAt(text).findViewById<EditText>(R.id.add_recette_ingredient_ET).text.toString()
                if (oui==""){
                    vide=true
                }
            }
            for (text in 0 until binding.list2.childCount) {
                var oui=binding.list2.getChildAt(text).findViewById<EditText>(R.id.add_recette_ingredient_ET).text.toString()
                if (oui==""){
                    vide=true
                }
            }
            if (vide){
                Toast.makeText(context, "no text inserted", Toast.LENGTH_LONG).show()
            }
            else{
                ...
                val ref= FirebaseDatabase.getInstance().getReference("Data").child("Recipe")
                ...
                val maxIng = binding.list1.childCount
                val maxStep = binding.list2.childCount

                for (text in 0 until maxIng) {
                ...
                for (text in 0 until maxStep)
                    userMap["step $text"]=binding.list2.getChildAt(text).findViewById<EditText>(R.id.add_recette_ingredient_ET).text.toString().toLowerCase()

                userMap["nbIngredients"] = maxIng.toString()
